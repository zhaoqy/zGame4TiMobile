...
game.prototype = {
	...
	*/				
        ...
        if (canvas.getContext) {
            ...
            this.gridSize = 45;
            myInterval = setInterval(make, 20)
        };
        ...
        function random_color() {
            ...
            return 'rgba(' + (rint >> 16) + ',' + (rint >> 8 & 255) + ',' + (rint & 255) + ',0.5)'
        }
    }
};
...
//        _('cur').setAttribute('class', 'slideOut');
    ...
    function lose() {
		...
		*/			
        ...
        _('results').innerHTML = '你输了!</sub>';
        _('results').innerHTML += '<br /><button onClick="stopWin();start(lvl)">重试</button>'
    };
...
//        _('cur').innerHTML = '结果: ' + g.playerTotal;
        if (g.playerTotal == ans && checkWin()) {
            clearBoard();
            playAudio('win');
            g.celebrate(true);
            _('results').style.display = 'block';
            _('results').innerHTML = '你赢了!';
